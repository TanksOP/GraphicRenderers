...
//float dist = 35.0f;
//float camAngleX = 202.189f;
//float camAngleY = -83.130f;
...
//float filmApertureWidth = 36;
//float filmApertureHeight = 24;
...
Matrix44f lookAt(const Vec3f from, const Vec3f to, const Vec3f _tmp = Vec3f(0, 1, 0))
{
    ...
    Vec3f forward = normalize(from - to); // z axis

    Vec3f right = crossProduct(normalize(_tmp), forward); // x axis

    Vec3f up = crossProduct(forward, right);// y axis
  ...
  // camToWorld = orientation* translation;
...
int main(int argc, char **argv)
{
    ...
    /*if (2 == argc) {
        ...
        model = new Model("Assets/front_vase.obj");
    }*/

    //model = new Model("Assets/front_vase.obj");

    models.emplace_back(new Model("Assets/front_vase.obj"));
    models.emplace_back(new Model("Assets/back_vase.obj"));
   // models.emplace_back(new Model("Assets/flower1.obj"));
    //models.emplace_back(new Model("Assets/flower2.obj"));
    models.emplace_back(new Model("Assets/flowertop1.obj"));
    models.emplace_back(new Model("Assets/flowertop2.obj"));
    models.emplace_back(new Model("Assets/rosestem1.obj"));
    models.emplace_back(new Model("Assets/rosestem2.obj"));
    models.emplace_back(new Model("Assets/table.obj"));
    models.emplace_back(new Model("Assets/bowl.obj"));
    models.emplace_back(new Model("Assets/background.obj"));
    models.emplace_back(new Model("Assets/glass.obj"));
    models.emplace_back(new Model("Assets/orange1.obj"));
    models.emplace_back(new Model("Assets/orange2.obj"));
    models.emplace_back(new Model("Assets/orange3.obj"));
    models.emplace_back(new Model("Assets/orange4.obj"));
    models.emplace_back(new Model("Assets/orange5.obj"));
    models.emplace_back(new Model("Assets/orange6.obj"));
    models.emplace_back(new Model("Assets/orange7.obj"));
    models.emplace_back(new Model("Assets/orange8.obj"));
    models.emplace_back(new Model("Assets/orange9.obj"));
    ...
    while (running) {
        ...
        /*camX = dist * -sin(camAngleX *(M_PI/180)) * cos((camAngleY) * (M_PI/180));
        camY = dist * -sin(camAngleY * (M_PI / 180));
        camZ = dist * -cos(camAngleX * (M_PI / 180)) * cos((camAngleY) * (M_PI / 180));   */    
       ...
       /* else {
        ...
        for (int j = 0; j < models.size(); j++) {
            ...
            for (uint32_t i = 0; i < models[j]->nfaces(); ++i) {
                // v0, v1 and v2 store the vertex positions of every vertex of the 3D model
                const Vec3f& v0 = models[j]->vert(models[j]->face(i)[0]);
                const Vec3f& v1 = models[j]->vert(models[j]->face(i)[1]);
                const Vec3f& v2 = models[j]->vert(models[j]->face(i)[2]);
                ...
                Vec2f st0 = models[j]->vt(models[j]->face(i)[0]);
                Vec2f st1 = models[j]->vt(models[j]->face(i)[1]);
                Vec2f st2 = models[j]->vt(models[j]->face(i)[2]);
                    ...
                    for (uint32_t x = x0; x <= x1; ++x) {
                        ...
                        if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
                            ...
                            if (z < depthBuffer[y * imageWidth + x]) { // is this triangle closer than others previously?
                                ...
                                TGAColor tgaCol(nDotView * 255, nDotView * 255, nDotView * 255, 255);
       ...
       // imageOut.flip_vertically();
        imageOut.write_tga_file("SoftwareRasterizerOutput.tga");

        // Calculate frame interval timing
        auto t_end = std::chrono::high_resolution_clock::now();
        auto passedTime = std::chrono::duration<double, std::milli>(t_end - t_start).count();
        // un comment to see the fram render time
